...
export default function Toolbar({
  onExport, onImport, onReset, onUndo, onRedo, canUndo, canRedo,
  darkMode, onToggleDark, user, onSignOut, onGoHome, flowName,
}) {
  ...
  return (
    <div className="toolbar">
      <div className="toolbar-title">
        {onGoHome ? (
          <>
            <button className="toolbar-title-link" onClick={onGoHome}>Tracer</button>
            {flowName && <span className="toolbar-breadcrumb"> / {flowName}</span>}
          </>
        ) : (
          'Tracer'
        )}
